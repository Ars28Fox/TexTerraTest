...
export function validateForm() {
	...
	const removeTextError = function(closestLabel) {
	...
	const changeTextError = function(item, closestLabel, inputValue, ruleType = null) {
		let text = '';
		if (ruleType != null) {
	...
	const addClass = function(item, className) {
		item.classList.add(className);
	};

	const removeClass = function(item, className) {
		item.classList.remove(className);
	};

	const allFormBtns = Array.from(document.querySelectorAll('input[type="submit"]'));
	const allValidateInputs = Array.from(document.querySelectorAll('.js-validation'));

	const checkInput = function(item, closestLabel, rules, ruleType) {
		let inputValue = item.value;
		let checkRequired = false;
		item.getAttribute('required') == '' ? (checkRequired = true) : (checkRequired = false);
		let regexp = rules;
		const validateFunc = regularTypes[ruleType][1];
		...
		} else if (inputValue.match(regexp) && regularTypes[item.dataset.rules].length > 1) {
			validateFunc(inputValue, textError)
				? removeClass(closestLabel, 'validate-error')
				: addClass(closestLabel, 'validate-error');
			changeTextError(item, closestLabel, inputValue, ruleType);
	...
	const checkAllInputs = function(closestForm, formBtn) {
